...
server and stored to a local folder *data/massive-ftp.ucsd.edu/v04* in the
user's home directory. Below we define the path to the data files and load the
data set. This needs to be adapted if the files were stored to a different
folder.
...
we use the *sample_name* for parameter `sampleGroup`. Combined with setting
`minFraction = 0.6`, an LC-MS feature is defined if a chromatographic peak is
present in at least 60% of the replicates per sample.
