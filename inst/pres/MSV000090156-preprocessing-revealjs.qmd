...
![](images/xcms4gnps2-source-code.png){width="700"}

## Available material

https://jorainer.github.io/xcms4gnps2
![](images/xcms4gnps-rendered.png){width="700"}

## Available material

https://jorainer.github.io/xcms4gnps2
![](images/xcms4gnps2-workflow.png){width="700"}
...
::: {.incremental}
- **Plan**: perform & explain preprocessing of LC-MS/MS data using *xcms*.
- Process feature's MS2 spectra and export results for FBMN with GNPS.
- Data: part of the MassIVE [MSV000090156](https://massive.ucsd.edu/ProteoSAFe/dataset.jsp?task=06bd49807caa4390961fb827606a8696)
  data set.
:::
...
pd <- data.frame(
    ...
    sample_name = c("A15M", "A15M", "A15M", "A45M", "A45M",
                    "A45M", "A5M", "A5M", "A5M", "M", "M",
                    "M", "PPL"),
    sample_desc = c("A15M_R1", "A15M_R2", "A15M_R3", "A45M_R1",
                    "A45M_R2", "A45M_R3", "A5M_R1", "A5M_R2",
...
#| echo: true
#| eval: false
#| code-line-numbers: "1-4|6-10|12-15"
...
- Key parameter: `peakwidth`: expected lower and upper retention time width of
...
#| eval: false
#| echo: true
#| code-line-numbers: "1|1-5|7-8|10-13"
...
- We use the *peak groups* method of *xcms*:
  ...
  - *peak groups* method: minimizes retention time differences of **anchor
    peaks**
...
::: {.fragment .fade-in}
- :point_up: *peak groups* method requires chrom peaks grouped across samples to
  define *anchor peaks*.
- :point_right: Need to perform an *initial* **correspondence analysis** to
  define these.
:::
...
- **lower panel**: retention time of identified chromatographic peaks along RT
  axis
- **black solid line**: density distribution of chrom peaks along retention
  times
...
- Increase `bw` to allow larger shifts in retention time between samples
...
- In general: use relaxed RT similarity for *initial* correspondence
:::
...
- :point_up: evaluate alignment result!
...
Visualizes adjusted RT (x-axis) against difference raw - adjusted RT (y-axis)
per sample. *Lines*: samples, *dots*: RT of anchor peaks.
:::
...
- Re-use the settings from the *initial* correspondence, except:
  ...
  - set `minFraction = 0.67`: feature defined if signal present in 2 out of 3
    replicates :point_right: sample-specific features
...
::: {.callout-warning .center appearance="simple" .fragment .fade-in}
Why? Also due to our choices for `sampleGroups` and `minFraction`: defined
...
mse <- fillChromPeaks(mse, param = cpap)
```
...
- :metal: highly customizable analyses through integration with R and packages
  from the *RforMassSpectrometry* initiative (+ Bioconductor)
- :muscle: large-scale data support: memory-saving and scalable

## Acknowledgments :raised_hands:

::: {.absolute top=200 left=0}
- All *RforMassSpectrometry* [contributors](https://www.rformassspectrometry.org/contributions/) :family:
- Philippine Louail :muscle:
- Steffen Neumann :pray:
:::

![](images/EULogo.jpg){.absolute bottom=58 left=750 height="60"}

::: {.absolute bottom=40 left=770}
...
:link: [https://jorainer.github.io/xcms4gnps2](https://jorainer.github.io/xcms4gnps2)\
:link: [https://RforMassSpectrometry.github.io/Metabonaut](https://RforMassSpectrometry.github.com/Metabonaut)
:::

. . .

![](images/xcms-paper.png){.absolute top=280 left=300 height="150"}

::: {.absolute top=430 left=300}
::: {style="font-size:16px"}
[https://doi.org/10.1021/acs.analchem.5c04338](https://doi.org/10.1021/acs.analchem.5c04338)
:::
:::
