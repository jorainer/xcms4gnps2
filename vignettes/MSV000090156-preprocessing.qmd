...
backend, which fetches the MS data files directly from the MassIVE FTP server
and caches them locally via *BiocFileCache*, so subsequent runs do not
...
The mass content seems to be comparable between samples, except for the *PPM*
...
[Metabonaut](https://rformassspectrometry.github.io/metabonaut) for more details
on *xcms*-based preprocessing.
:::
...
:information_source: note that in most cases it is not necessary that all
samples are perfectly aligned. Some variation in retention time can be accounted
for in the final correspondence analysis.
...
Next to the main fragment peaks also some very low intensity peaks are
visible. We

```{r}
#' define a function to keep only fragments with an intensity > 1%
#' of the max fragment intensity
min_intensity <- function(x, ...) {
    x > max(x, na.rm = TRUE) * 0.01
}

#' apply this filter to the MS2 spectra and remove spectra with
#' less than 3 peaks
ms2 <- filterIntensity(ms2, intensity = min_intensity)
ms2 <- ms2[lengths(ms2) > 2]
```

All MS2 spectra in [@fig-ms2-1] look similar - to get some quantitative
measurement for this similarity we next calculate also a pairwise
